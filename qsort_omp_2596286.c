...
void par_qsort_sec(int p, int r, float *data){
	if(p < r){
        int q = partition(p, r, data);
...
void par_qsort_task(int p, int r, float *data){
    if(p < r){
        int q = partition(p, r, data);
#pragma omp task
        par_qsort_task(p, q-1, data);
#pragma omp task
        par_qsort_task(q+1, r, data);
//#pragma omp nowait
    }
}
